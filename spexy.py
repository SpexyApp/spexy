...
while True:
    ...
    #Disk Usage Info
    dsk = psutil.disk_partitions(all=False)
    diskIO = psutil.disk_io_counters(perdisk=False)
    sysinfo["disk"] = []
    for i in range(len(dsk)):
        diskUsage = psutil.disk_usage(dsk[i].device)
        sysinfo['disk'].append({
        'device':dsk[i].device,
        'mtpoint':dsk[i].mountpoint,
        'fsystem':dsk[i].fstype,
        'totspace':float("{0:.3f}".format(diskUsage.total / 1024 ** 3)),
        'usedspace':float("{0:.3f}".format(diskUsage.used / 1024 ** 3)),
        'freespace':float("{0:.3f}".format(diskUsage.free / 1024 ** 3))})
    ...
    try:
        ...
        response = requests.post("http://nisarg.me:1337/sendinfo/" + str("testmac"),data=payload)
        payload = {'json_payload': json.dumps(processes)}
        response = requests.post("http://nisarg.me:1337/sendtasks/" + str("testmac"),data=payload)

        r = requests.get("http://nisarg.me:1337/command/"+"testmac")
        cmdlist = r.json()
        print (cmdlist)
        action = cmdlist['action']
        cmd = cmdlist['cmd']

        if(action == 1):
            p = psutil.Process(pid=int(cmd))
            p.kill()
        elif(action == 2):
            subprocess.call(cmd)
        else:
